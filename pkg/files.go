...
func FileStats(filePath string, isRemote bool, sshConfig *SSHConfig) (int, int64, error) {
	...
	var linesCount int
	for {
		// ReadLine reads until the next newline or EOF
		_, isPrefix, err := reader.ReadLine()
		if err != nil {
			if err == io.EOF {
				break // End of file
			}
			slog.Error("failed to read line", "filePath", filePath, "error", err)
			return 0, 0, err
		}

		// If isPrefix is true, the line was too long for the buffer.
		// We can continue reading the rest of the line in chunks.
		if !isPrefix {
			linesCount++
		}
	}
